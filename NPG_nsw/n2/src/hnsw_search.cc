...
namespace n2
{
    ...
    //NANG-search
    template <typename DistFuncType>
    void HnswSearchImpl<DistFuncType>::SearchByVector(const std::vector<float> &qvec, size_t k, int ef_search,
                                                      std::vector<int> &result)
    {
        resetdisc();
        SearchByVector_(qvec, k, ef_search, result);
    }
    ...
    //NANG-search
    //template <typename DistFuncType>
    //void HnswSearchImpl<DistFuncType>::SearchByVector_(const vector<float> &qvec, size_t K, int search_L, std::vector<unsigned> &indices, unsigned nd_)
    //{
    //    unsigned L = search_L;
    //    std::vector<Neighbor> retset(L + 1);
    //    std::vector<unsigned> init_ids(L);
    //    std::mt19937 rng(rand());
    //    GenRandom(rng, init_ids.data(), L, nd_);
    ...
    //        if (id >= nd_)
    //            continue;
    //        const float *vec = (const float *)(model_level0_node_base_offset_ + id * memory_per_node_level0_);
    //        _mm_prefetch(vec, _MM_HINT_NTA);
    //        float dist = dist_func_(qraw, vec, data_dim_);
    //        //dist_cout++;
    ...
    //
    //            const int *friends_with_size = (const int *)(model_level0_ + n * memory_per_node_level0_ + sizeof(int));
    //            _mm_prefetch(friends_with_size, _MM_HINT_T0);
    //            int MaxM = friends_with_size[0] / 2;
    //
    //            for (unsigned m = 1; m <= MaxM; ++m)
    //            {
    //                unsigned id = friends_with_size[m];
    //                if (flags[id])
    //                    continue;
    //                flags[id] = 1;
    //
    ...
    template <typename DistFuncType>
    void HnswSearchImpl<DistFuncType>::SearchByVector_(const vector<float> &qvec, size_t k, int ef_search, std::vector<int> &result)
    {
        ...
        while (!candidates.empty())
        {
            ...
            candidates.pop();
            const int *friends_with_size = (const int *)(model_level0_ + cur_node_id * memory_per_node_level0_);
            _mm_prefetch(friends_with_size, _MM_HINT_T0);
            int size = friends_with_size[0];

            for (auto j = 2; j <= size + 1; ++j)
            {
                _mm_prefetch(visited + friends_with_size[j], _MM_HINT_T0);
            }
            for (auto j = 2; j <= size + 1; ++j)
        ...
        //std::cout << found_distances.size() << std::endl;
        //found_distances.clear();
        //candidates.clear();
        //candidates.swap(visited_nodes);
        //found_distances.emplace(cur_dist);
        //candidates.emplace(cur_node_id, cur_dist);
        //while(found_distances.size()>)
        size_t kk = 0;
        while (!visited_nodes.empty() and kk < k)
        {
            kk++;
            const IdDistancePair &c = visited_nodes.top();
            if (c.second > found_distances.top())
            {
                break;
            }
            ...
            cur_node_id = c.first;
            candidates.emplace(std::move(const_cast<IdDistancePair &>(c)));
            visited_nodes.pop();

            const int *friends_with_size = (const int *)(model_level0_ + cur_node_id * memory_per_node_level0_);
            _mm_prefetch(friends_with_size, _MM_HINT_T0);
            int size = friends_with_size[1];
            ...
            for (auto j = 2; j <= size + 1; ++j)
            {
                ...
                if (visited[node_id] != visited_mark)
                {
                    ...
                    if (d < found_distances.top() || found_distances.size() < ef_search)
                    {
                        visited_nodes.emplace(node_id, d);
                        found_distances.emplace(d);
                        if (found_distances.size() > ef_search)
                        {
                            found_distances.pop();
